...
class WaypointMissionManager(Node):
    ...
    def create_sample_mission(
        self, home_lat=-35.363262, home_lon=149.165237, home_alt=2.0
    ):
        """
        ...
        wp4 = self.create_waypoint(
            home_lat, home_lon + offset, square_alt, frame=3, command=16
        )
        ...
        if future.result() is not None:
            ...
            if response.success:
                self.get_logger().info(
                    f"Successfully pushed {response.wp_transfered} waypoints"
                )
                return True
            else:
    ...
    def pull_waypoints(self):
        ...
        if future.result() is not None:
            ...
            if response.success:
                self.get_logger().info(
                    f"Successfully pulled {response.wp_received} waypoints"
                )
            else:
