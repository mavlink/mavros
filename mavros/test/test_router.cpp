...
{
public:
  Router::SharedPtr router;

  MockEndpoint::SharedPtr fcu1;
  MockEndpoint::SharedPtr fcu2;
  MockEndpoint::SharedPtr uas1;
  MockEndpoint::SharedPtr uas2;
  MockEndpoint::SharedPtr gcs1;
  ...
  {
    reset();
    this->router = std::make_shared<Router>("test_mavros_router");

    auto make_and_add_mock_endpoint =
      [this](id_t id, const std::string & url, LT type,
        std::set<addr_t> remotes) -> auto {
        ...
        this->router->endpoints[id] = ep;

        return ep;
      };

    this->fcu1 = make_and_add_mock_endpoint(1000, "mock://fcu1", LT::fcu, {0x0000, 0x0100, 0x0101});
    this->fcu2 = make_and_add_mock_endpoint(1001, "mock://fcu2", LT::fcu, {0x0000, 0x0200, 0x0201});
    this->uas1 = make_and_add_mock_endpoint(1002, "/uas1_1", LT::uas, {0x0000, 0x0100, 0x01BF});
    this->uas2 = make_and_add_mock_endpoint(1003, "/uas2_1", LT::uas, {0x0000, 0x0200, 0x02BF});
    this->gcs1 = make_and_add_mock_endpoint(1004, "mock://gcs1", LT::gcs, {0x0000, 0xFF00, 0xFFBE});
  }
