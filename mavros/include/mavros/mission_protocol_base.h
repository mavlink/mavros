...
namespace mavros {
  namespace plugin {
    ...
    template < >
    inline WP_ITEM_INT mav_from_msg(
      const mavros_msgs::Waypoint & wp, const uint16_t seq,
      WP_TYPE type)
    {
      WP_ITEM_INT ret {};
    ...
    std::string waypoint_to_string(const ITEM & wp)
    {
      std::stringstream ss;
      ss.precision(7);
      ss << '#' << wp.seq << (wp.current ? '*' : ' ') << " F:" << wp.frame << " C:" <<
        std::setw(3) << wp.command;
...
public:
      ...
      virtual void initialize(ros::NodeHandle * _wp_nh)
      {
        ...
        schedule_timer = _wp_nh->createTimer(
          BOOTUP_TIME_DT, &MissionBase::scheduled_pull_cb, this,
          true);
        schedule_timer.stop();
      }
...
protected:
      ...
      {
        ROS_DEBUG_NAMED(
          log_ns, "%s:m: write partial list %u - %u",
