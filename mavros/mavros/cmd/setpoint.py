...
def do_local_pos(args):
    ...
    pos = sp.PoseStamped(
        header=sp.Header(frame_id='mavsetp', stamp=rospy.get_rostime()))
    pos.pose.position = sp.Point(x=args.position[0],
                                 y=args.position[1],
                                 z=args.position[2])
...
def do_local_vel(args):
    ...
    vel = sp.TwistStamped(
        header=sp.Header(frame_id='mavsetp', stamp=rospy.get_rostime()))
    vel.twist.linear = sp.Vector3(x=args.velocity[0],
                                  y=args.velocity[1],
                                  z=args.velocity[2])
...
def do_local_accel(args):
    ...
    vel = sp.Vector3Stamped(
        header=sp.Header(frame_id='mavsetp', stamp=rospy.get_rostime()))
    vel.vector = sp.Vector3(x=args.acceleration[0],
                            y=args.acceleration[1],
                            z=args.acceleration[2])
...
def main():
    parser = argparse.ArgumentParser(
        description="Command line tool for controlling the device by setpoints."
    )
    ...
    local_group.add_argument('-p',
                             '--position',
                             type=float,
                             nargs=4,
                             metavar=('x', 'y', 'z', 'y_ang'),
                             help="Local position & desired yaw angle")

    local_group.add_argument('-v',
                             '--velocity',
                             type=float,
                             nargs=4,
                             metavar=('vx', 'vy', 'vz', 'y_rate'),
                             help="Linear velocity & desired yaw rate")

    local_group.add_argument('-a',
                             '--acceleration',
                             type=float,
                             nargs=3,
                             metavar=('afx', 'afy', 'afz'),
                             help="Linear acceleration/force")
