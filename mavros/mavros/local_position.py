...
class LocalPositionPlugin(PluginModule):
    ...
    def subscribe_pose(self,
        ...
        return self.create_subscription(PoseStamped,
                                        ('local_position', 'pose'), callback,
                                        qos_profile)

    def subscribe_pose_cov(self,
                           callback: SubscriptionCallable,
                           qos_profile=SENSOR_QOS) -> rclpy.node.Subscription:
        return self.create_subscription(PoseWithCovarianceStamped,
    ...
    def subscribe_velocity_body_cov(
        ...
        return self.create_subscription(
            TwistWithCovarianceStamped,
            ('local_position', 'velocity_body_cov'), callback, qos_profile)

    def subscribe_accel(self,
                        callback: SubscriptionCallable,
                        qos_profile=SENSOR_QOS) -> rclpy.node.Subscription:
        return self.create_subscription(AccelWithCovarianceStamped,
                                        ('local_position', 'accel'), callback,
                                        qos_profile)

    def subscribe_odom(self,
                       callback: SubscriptionCallable,
                       qos_profile=SENSOR_QOS) -> rclpy.node.Subscription:
        return self.create_subscription(Odometry, ('local_position', 'local'),
                                        callback, qos_profile)
