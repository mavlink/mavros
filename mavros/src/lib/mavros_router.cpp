...
static std::atomic<id_t> id_counter { 0 };

void Router::route_message(Endpoint::SharedPtr src, const mavlink_message_t* msg, const Framing framing)
{
}

void Router::add_endpoint(const mavros_msgs::srv::EndpointAdd::Request::SharedPtr request, mavros_msgs::srv::EndpointAdd::Response::SharedPtr response)
{
}

void Router::del_endpoint(const mavros_msgs::srv::EndpointDel::Request::SharedPtr request, mavros_msgs::srv::EndpointDel::Response::SharedPtr response)
{
}
...
void Endpoint::recv_message(const mavlink_message_t* msg, const Framing framing)
{
    ...
    if (auto nh = this->parent.lock()) {
        if (sp.second || scp.second) {
            RCLCPP_INFO(nh->get_logger(), "link[%d] detected remote address %d.%d", this->id, msg->sysid, msg->compid);
        }
...
bool ROSEndpoint::open()
{
    ...
    this->to = nh->create_subscription<mavros_msgs::msg::Mavlink>(utils::format("%s/%s", this->url, "mavlink_to"), QoS(1000).best_effort(),
