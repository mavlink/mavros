  ...
  plugin_subscriptions{}
{
  ...
  mavlink_sub = mavlink_nh.subscribe(
    "to", 100, &MavRos::mavlink_sub_cb, this,
    ros::TransportHints()
  ...
  mav_uas.add_connection_change_handler(
    std::bind(
      &MavlinkDiag::set_connection_status,
      &fcu_link_diag, std::placeholders::_1));
  mav_uas.add_connection_change_handler(
    std::bind(
      &MavRos::log_connect_change, this,
      std::placeholders::_1));
  ...
  if (gcs_link) {
    ...
    gcs_link->message_received_cb =
      [this, fcu_link](const mavlink_message_t * msg, const Framing framing) {
...
static bool is_blacklisted(
  std::string & pl_name, ros::V_string & blacklist,
