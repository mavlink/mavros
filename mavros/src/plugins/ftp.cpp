...
static uint32_t crc32(uint8_t *buff, size_t len)
{
	uint32_t state = 0;
	for (size_t i = 0; i < len; i++)
		state = crc32_table[(state ^ buff[i]) & 0xff] ^ (state >> 8);
}
// XXX: end temp
...
public:
	...
	void send(UAS *uas, uint16_t seqNumber) {
		...
		hdr->padding[1] = 0;
		hdr->padding[2] = 0;
		hdr->crc32 = crc32(raw_payload(), hdr->size + sizeof(PayloadHeader));
...
private:
	...
	void handle_file_transfer_protocol(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
		...
		if (incoming_seqnr != expected_seqnr) {
			ROS_WARN_NAMED("ftp", "FTP: Lost sync! seqnr: %u != %u", incoming_seqnr, expected_seqnr);
			// TODO: reset
		}
	...
	void handle_req_ack(FTPRequest &req) {
		...
		case OP_ACK: op_state = OP_IDLE;	break;
