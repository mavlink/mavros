...
std::string waypoint_to_string(const ITEM & wp)
{
  ...
  ss << '#' << wp.seq << (wp.current ? '*' : ' ') << " F:" << wp.frame << " C:" << std::setw(3) <<
    wp.command;
  ss << " p: " << wp.param1 << ' ' << wp.param2 << ' ' << wp.param3 << ' ' << wp.param4 << " x: " <<
    wp.x << " y: " << wp.y << " z: " << wp.z;
  return ss.str();
}
...
public:
  ...
  void initialize(UAS & uas_) override
  {
    ...
    wp_timer.stop();
    schedule_timer = wp_nh.createTimer(
      BOOTUP_TIME_DT, &WaypointPlugin::scheduled_pull_cb, this,
      true);
    schedule_timer.stop();
...
private:
  ...
  void handle_mission_item_int(const mavlink::mavlink_message_t * msg, WP_ITEM_INT & wpi)
  {
    ...
    if (wp_state == WP::RXWPINT) {
      if (wpi.seq != wp_cur_id) {
        ROS_WARN_NAMED(
          "wp", "WP: Seq mismatch, dropping item (%d != %zu)",
          wpi.seq, wp_cur_id);
        return;
      }
  ...
  bool sequence_mismatch(const uint16_t & seq)
  {
    if (seq != wp_cur_id && seq != wp_cur_id + 1) {
      ROS_WARN_NAMED(
        "wp", "WP: Seq mismatch, dropping request (%d != %zu)",
        seq, wp_cur_id);
      return true;
    } else {
      return false;
