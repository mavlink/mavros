...
private:
	...
	void mocap_tf_cb(const geometry_msgs::TransformStamped::ConstPtr &trans)
	{
		...
		send_fake_gps(trans->header.stamp, ftf::transform_frame_enu_ecef(map_origin, Eigen::Vector3d(pos_enu.translation())));
	}
	...
	void mocap_pose_cb(const geometry_msgs::PoseStamped::ConstPtr &req)
	{
		...
		send_fake_gps(req->header.stamp, ftf::transform_frame_enu_ecef(map_origin, Eigen::Vector3d(pos_enu.translation())));
	}
