...
DistanceSensorItem::Ptr DistanceSensorItem::create_item(
  DistanceSensorPlugin * owner,
  std::string topic_name)
{
  ...
  if (!p->is_subscriber) {
    ...
    if (p->orientation == -1 && !orientation_str.empty()) {
      ROS_ERROR_NAMED(
        "distance_sensor", "DS: %s: defined orientation (%s) is not valid!",
    ...
    if (p->send_tf) {                   // sensor position defined if 'send_tf' set to TRUE
      ...
      ROS_DEBUG_NAMED(
        "sensor_position", "DS: %s: Sensor position at: %f, %f, %f", topic_name.c_str(),
